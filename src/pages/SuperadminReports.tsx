...
const ReportCard = ({ title, value, subValue, trend, icon: Icon, delay }: any) => (
  ...
  >
    <div className="flex justify-between items-start mb-6">
      <div className="p-3 bg-[#000051]/5 rounded-2xl text-[#7F00DF]">
        <Icon size={24} />
      </div>
      {trend && (
        <span className={`text-[10px] font-black px-3 py-1 rounded-full ${trend > 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-indigo-50 text-[#7F00DF]'}`}>
          {trend > 0 ? '+' : ''}{trend}%
        </span>
      )}
    </div>
  ...
  return (
    <Layout role="SUPERADMIN">
      <div className="p-4 md:p-12 w-full max-w-[1600px] mx-auto flex flex-col gap-10 bg-[#F2F2F2] min-h-screen">
        <header className="flex flex-col items-start lg:flex-row lg:items-end justify-between gap-6 pt-12 lg:pt-0">
          ...
          <div className="flex items-center justify-end gap-3 w-full lg:w-auto overflow-x-auto no-scrollbar pb-2 lg:pb-0">
            ...
            <div className="relative w-full sm:w-auto">
              <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 text-[#7F00DF]" size={18} />
              <select 
            ...
            <button className="w-full sm:w-auto bg-[#000051] text-white px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-[#7F00DF] transition-all shadow-xl shadow-[#000051]/20">
              <Download size={18} />
              Exportar
            </button>
          </div>
            ...
            <div className="flex items-center justify-between mb-10">
              ...
              <div className="flex gap-4">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-[#000051]" />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={MOCK_MONTHLY_DATA}>
                  ...
                  <Tooltip 
                    ...
                    cursor={{ fill: '#f8fafc' }}
                  />
                  <Bar dataKey="aprobadas" fill="#7F00DF" radius={[6, 6, 0, 0]} barSize={30} />
        ...
        <div className="bg-black p-10 md:p-16 rounded-[60px] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-12 overflow-hidden relative">
          <div className="absolute top-0 right-0 w-96 h-96 bg-[#000051]/10 blur-[100px] rounded-full -mr-48 -mt-48" />
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto relative z-10">
            ...
            <button className="flex-1 md:flex-none px-10 py-5 bg-[#000051] text-white rounded-[24px] font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:scale-[1.02] transition-all shadow-xl">
              <FileText size={18} />
              PDF (.pdf)
            </button>
          </div>
